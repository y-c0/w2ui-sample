var CONST_W2UI_ALL_FIELDS = "絞り込み";
...
var w2utils = (function ($) {
    ...
    var obj = {
        ...
        settings : {
            ...
            "dateStartYear"     : 2000,       // start year for date-picker
            "dateEndYear"       : nowY        // end year for date picker
        },
        isBin           : isBin,
        isInt           : isInt,
        isFloat         : isFloat,
...
(function ($) {
    ...
    w2grid.prototype = {
        ...
        operators: { // for search fields
            ...
            "textmulti"    : ['ismulti', 'beginsmulti', 'containsmulti', 'endsmulti'],
                ...
                if (value != null) {
                    if (field.toLowerCase() == 'all') {
                        ...
                        if (this.searches.length > 0) {
                            for (var i = 0; i < this.searches.length; i++) {
                                var search = this.searches[i];
                                if (    search.type == 'text' || search.type == 'textmulti' || (search.type == 'alphanumeric' && w2utils.isAlphaNumeric(value))
        ...
        searchReset: function (noRefresh) {
            ...
            if (this.searches.length > 0) {
                if (!this.multiSearch || !this.show.searchAll) {
                    ...
                    this.last.field   = 'all';
                    this.last.caption = w2utils.lang(CONST_W2UI_ALL_FIELDS);
                }
            }
        ...
        searchShowFields: function () {
            ...
            for (var s = -1; s < this.searches.length; s++) {
                ...
                if (s == -1) {
                    ...
                    search = { field: 'all', caption: w2utils.lang(CONST_W2UI_ALL_FIELDS) };
                } else {
        ...
        initAllField: function (field, value) {
            var el = $('#grid_'+ this.name +'_search_all');
            if (field == 'all') {
                var search = { field: 'all', caption: w2utils.lang(CONST_W2UI_ALL_FIELDS) };
                el.w2field('clear');
                el.change();
            } else {
                var search = this.getSearch(field);
        ...
        request: function (cmd, add_params, url, callBack) {
            ...
            if (this.last.xhr_offset === 0) {
                ...
                if (this.autoLoad === true) {
                    more.show().find('td').html('<div style="position:absolute; width:100%; "><div style="width: 20px; height: 20px; " class="w2ui-spinner"></div></div>');
                } else {
                        ...
                        if (cmd == 'get') {
                            ...
                            if (this.last.xhr_offset === 0) {
                                ...
                                if (data.total != -1 && parseInt(data.total) != parseInt(this.total)) {
                                	this.message(""+data.records.length+"///"+this.total);
        ...
        render: function (box) {
            ...
            $(this.box)
                ...
                .html('<div class="w2ui-grid-box">'+
                      ...
                      // custom by kikuchi'    <textarea id="grid_'+ this.name +'_focus" class="w2ui-grid-focus-input"></textarea>'+
                      '</div>');
        ...
        getSearchesHTML: function () {
            var obj  = this;
            var html = '<table cellspacing="0" onclick="event.stopPropagation()" style="background-color:#fcfcff;"><tbody>';
            var showBtn = false;
            for (var i = 0; i < this.searches.length; i++) {
            ...
            html += '<tr>'+
                    ...
                    '        <button class="w2ui-btn" onclick="obj = w2ui[\''+ this.name +'\']; if (obj) { obj.searchReset(false); }" style="width:100px; height:30px; font-family:MS UI Gothic; color:black; font-size:middle;">'+ w2utils.lang('リセット') + '</button>'+
                    '        <button class="w2ui-btn" onclick="obj = w2ui[\''+ this.name +'\']; if (obj) { obj.search(); }" style="width:100px; height:30px; font-family:MS UI Gothic; color:black; font-size:middle;">'+ w2utils.lang('検索') + '</button>'+
                    '        </div>'+
                    '    </td>'+
                    '</tr></tbody></table>';
            return html;
...
//                    break;
        ...
        status: function (msg) {
            if (msg != null) {
                ...
                if (sel.length >= 1) this.toolbar.enable('w2ui-delete'); else this.toolbar.disable('w2ui-delete');
                // custom by okada
                if(typeof CUSTOM_TOOLBARS !== 'undefined') {
                    if (sel.length >= 1) for (var cidx in CUSTOM_TOOLBARS) this.toolbar.enable(CUSTOM_TOOLBARS[cidx]);
                    else                 for (var cidx in CUSTOM_TOOLBARS) this.toolbar.disable(CUSTOM_TOOLBARS[cidx]);
                }
            }
        },
        ...
        init: function () {
            ...
            switch (this.type) {
                ...
                case 'hex':
                    ...
                    if (['text', 'textmulti', 'alphanumeric', 'hex', 'bin'].indexOf(this.type) != -1) {
